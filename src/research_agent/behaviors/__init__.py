...
and strategy_planner built via factories. Safe to call repeatedly.
...
is already in the registry. Our job in `register_all()`:
  ...
  3. The factory-built behaviors (decomposer, strategy_planner) are
     rebuilt by calling the factories; their @llm_behavior re-fires
     and auto-registers them.
  4. The module-level behaviors lost their registry slot at step 1, so
     we re-add them with explicit register() calls. We do NOT re-import
     the modules \u2014 the stable instance references work fine.
"""
...
def register_all(
    *,
    ...
    are rebuilt with the configured caps, so the LLM API rejects
    over-cap responses at parse time.
    """
    clear_registry()
    clear_tool_registry()
...
def behavior_count() -> int:
    """Total behaviors the agent registers (2 factory-built + module-level)."""
    return 2 + len(_MODULE_LEVEL_BEHAVIORS)


# Backward-compat: a list view of module-level behaviors only. The
# factory-built decomposer and strategy_planner are NOT instantiated
# at import time (each instantiation auto-registers and would leak).
# Use `register_all()` to get the live, fully-registered list.
